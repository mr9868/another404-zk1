...
import { Circuit, circuitArray, provable, cloneCircuitValue, getBlindingValue, memoizationContext, toConstant, Struct, } from './circuit_value.js';
import { Body, AccountUpdate, signJsonTransaction, ZkappCommand, Permissions, ZkappPublicInput, Events, SequenceEvents, Authorization, CallForest, TokenId, smartContractContext, zkAppProver, ZkappStateLength, } from './account_update.js';
...
// external API
export { SmartContract, method, deploy, signFeePayer, declareMethods, Callback, Account, VerificationKey, Reducer, };
...
Use the optional \`maxTransactionsWithActions\` argument to increase this number.`);
            ...
            for (let i = 0; i < maxTransactionsWithActions; i++) {
                ...
                let newStates = actionss.map((actions) => {
                    ...
                    let newState = Circuit.witness(stateType, () => {
                        ...
                        return fromFields(toFields(state).map((x) => x.toConstant()), toAuxiliary(state));
                        // return state;
                    });
        ...
        getActions({ fromActionHash, endActionHash, }) {
            ...
            Circuit.asProver(() => {
                ...
                event.actions.map((action) => reducer.actionType.fromFields(action.map((fieldAsString) => Field(fieldAsString)))));
            });
...
async function deploy(SmartContract, { zkappKey, verificationKey, initialBalance, feePayer, tokenId = TokenId.default, }) {
    ...
    let tx = await Mina.transaction(feePayer, () => {
        if (initialBalance !== undefined) {
            ...
            let amount = UInt64.from(String(initialBalance)).add(Mina.accountCreationFee());
...
function addFeePayer({ feePayer, accountUpdates, memo }, feePayerKey, { transactionFee = 0, feePayerNonce = undefined, memo: feePayerMemo = undefined, }) {
